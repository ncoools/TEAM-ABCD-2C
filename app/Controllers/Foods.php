...
class Foods extends Controller
{
    ...
    public function save(){
        ...
        $foodModel = new \App\Models\FoodsModel();
        ...
        if ($foodModel->insert($data)) {
            $logModel->addLog('New Foods has been added: ' . $name, 'ADD');
            return $this->response->setJSON(['status' => 'success']);
        } else {
            return $this->response->setJSON(['status' => 'error', 'message' => 'Failed to save Foods']);
        }
    }
...
public function delete($id){
    ...
    $user = $model->find($id);
    if (!$user) {
        return $this->response->setJSON(['success' => false, 'message' => 'Foods not found.']);
    }

    $deleted = $model->delete($id);

    if ($deleted) {
        $logModel->addLog('Delete Foods', 'DELETED');
        return $this->response->setJSON(['success' => true, 'message' => 'Foods deleted successfully.']);
    } else {
        return $this->response->setJSON(['success' => false, 'message' => 'Failed to delete Foods.']);
    }
}
